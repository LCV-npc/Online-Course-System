...
export default function InstructorGradeSubmissions() {
  const { allCourses, currentUser } = useApp();
  const [courseId, setCourseId] = useState<string>('');
  const [loading, setLoading] = useState(false);
  const [submissions, setSubmissions] = useState<Submission[]>([]);

  // Filter courses that belong to the instructor
  const instructorCourses = useMemo(() => {
    return allCourses.filter(c => c.instructorId === currentUser.id);
  }, [allCourses, currentUser.id]);

  const courseOptions = useMemo(() => {
    return [{ id: '', title: 'Tất cả khóa học của tôi' }, ...instructorCourses.map((c) => ({ id: c.id, title: c.title }))];
  }, [instructorCourses]);

  const load = async () => {
    setLoading(true);
    try {
      const url = courseId ? `/api/instructor/submissions?courseId=${encodeURIComponent(courseId)}` : '/api/instructor/submissions';
  ...
  const saveFor = async (sub: Submission) => {
    try {
      ...
      await apiFetch(`/api/instructor/submissions/${encodeURIComponent(sub.id)}`, {
        auth: true,
        method: 'PATCH',
        body: JSON.stringify({
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm">{submissions.length} bài nộp cần đánh giá</p>
        </div>
