...
const menuItems = [
  ...
  { to: '/admin/courses', label: 'Quản lý Khóa học', icon: <BookOpen className="w-5 h-5" /> },
  { to: '/admin/instructors', label: 'Quản lý Giảng viên', icon: <Users className="w-5 h-5" /> },
  { to: '/admin/students', label: 'Quản lý Học viên', icon: <Users className="w-5 h-5" /> },
  { to: '/admin/submissions', label: 'Chấm bài & phản hồi', icon: <BarChart3 className="w-5 h-5" /> },
];
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-10">
        <div className="w-full max-w-md bg-white rounded-3xl shadow-sm border border-gray-100 p-7 text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">
            Vui lòng đăng nhập bằng tài khoản admin để truy cập khu vực quản trị.
          </p>
          <Link
            to="/login"
        ...
        <div className="p-4 border-b border-gray-700 flex items-center justify-between">
          {sidebarOpen && (
            <div className="flex items-center gap-2">
              ...
              <span style={{ fontWeight: 700 }} className="text-white">EduPro Admin</span>
            </div>
          )}
          <button
